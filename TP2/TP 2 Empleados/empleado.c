...
int emp_mostrarIndice(Empleado* pEmpleado,int limite)
{
    ...
    if(pEmpleado != NULL && limite > 0)
    {
        if(emp_ordenarEmpleadoPorSectorApellido(pEmpleado,limite)==0)
        {
            for(i=0;i<limite;i++)
            {
                if(pEmpleado[i].isEmpty==0)
    ...
    if(pEmpleado != NULL && limite > 0)
    {
        ...
        if(pEmpleado[i].isEmpty==0)
        {
            do
            {
                ...
                {
                    if((pEmpleado[i].sector > pEmpleado[i+1].sector) ||
                        (pEmpleado[i].sector == pEmpleado[i+1].sector && strcmp(pEmpleado[i].lastName,pEmpleado[i+1].lastName)>0))
                    {
                        flagSwap=1;
                        auxEmpleado=pEmpleado[i];
                        pEmpleado[i]=pEmpleado[i+1];
                        pEmpleado[i+1]=auxEmpleado;
                    }
                }
            }while(flagSwap==1);
